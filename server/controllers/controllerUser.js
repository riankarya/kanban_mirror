...
class Controller {
    ...
    User.findOne({where: {email}})
      .then(data => {
        if (!data) {
          ...
          throw { name: 'UniqueEmailValidation', msg: 'email has already taken'}
        }
      })
      .then(data => {
        data = {
  ...
  static loginUser(req, res, next) {
    const { email, password } = req.body
    const obj = { email, password }
    console.log(obj, 'asup ti controlleruser')
    User.findOne({ where: { email } })
      .then(data => {
        console.log(data, 'asup ti then data')
        if (!data || hashPassword(password) != data.password) throw { name: 'UnAuthorized', msg: 'invalid email or password' }
        let payload = {
          id: data.id,
          email: data.email
        }
        console.log(payload, 'asup ti payload')
        const id = +data.id
        let token = generateToken(payload)
        console.log(token, 'asup ti token')
